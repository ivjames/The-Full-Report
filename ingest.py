...
def ingest_listings(limit: int | None, latest_only: bool) -> None:
    init_db()
    ...
    for path in SCRAPE_CONFIG.listing_paths:
        ...
        response.raise_for_status()
        all_entries.extend(parse_listing(response.text, SCRAPE_CONFIG.base_url))
...
def main() -> None:
    ...
    ingest_listings(limit=args.limit, latest_only=not args.all)
